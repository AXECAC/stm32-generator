...
use crate::core::{
    ...
    peripherals::{
        ...
        ethernet::{
            MacAddr,
            w5500::{NetworkConfig, SocketMode, W5500Config},
        },
    },
};
                ...
                if w5500_count < 2 {
                    ui.group(|ui| {
                        ...
                        egui::Grid::new("w5500_form").show(ui, |ui| {
                            ...
                            let mut pin_combo = |id: &str, label: &str, selected: &mut usize, local_used: &[usize]| {
                                ui.label(label);

                                let filtered_pins: Vec<_> = available_pins.iter()
                                    .filter(|(orig_i, _)| !local_used.contains(orig_i))
                                    .collect();

                                if filtered_pins.iter().find(|&&(orig_i, _)| *orig_i == *selected).is_none() && !filtered_pins.is_empty() {
                            ...
                            pin_combo("w5500_rst", "Пин RST:", &mut self.w5500_rst, &[self.w5500_cs]);
                        ...
                        if ui.button("Добавить W5500").clicked() {
                            ...
                            if !mac_valid {
                                ...
                                let w5500_cfg_res = W5500Config::new(
                                    chosen_spi,
                                    ChosenPin::StmF401(cs_val),
                                    ChosenPin::StmF401(rst_val),
                                    NetworkConfig {
                                    ...
                                    SocketMode::TcpServer { port: port.unwrap(), socket_num: 0 },
                                );

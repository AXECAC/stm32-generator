    ...
    pub fn render(&mut self, ui: &mut egui::Ui, config: &mut Config, page: &mut Page) {
        ...
        ui.horizontal(|ui| {
            ...
            ui.vertical(|ui| {
                ...
                ui.add_space(20.0);
                if ui.button("Next: SPI Buses ->").clicked() {
                    *page = Page::Spi;
                }
            });

            // Правая колонка - Картинка распиновки
            ui.add_space(20.0);
            ui.vertical(|ui| {
                ui.label("STM32F401 Black Pill Pinout:");
                ui.add(egui::Image::new(egui::include_image!("../../../assets/pinout.jpg")).max_width(500.0));
            });
        });
    }
}
